                ...
                <div style={{ maxWidth: 1200, width: '100%', margin: '0 auto', padding: '1.5rem 1rem' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '380px 1fr', gap: 16, alignItems: 'start' }}>
                        ...
                        <div style={{
                            ...
                            {/* Map legend removed per request — pins are self-explanatory via popups */}
                        </div>
                    </div>
                </div>
            </div>
...
function ResultsScreen({ swotResults, competitiveAnalysis, establishmentName, keyword, city, onBack }) {
  ...
  const downloadCombined = () => {
    ...
    const lines = [
      ...
      `Location: ${city}`,
      ``,
      // If narrative report exists, use it as the main content
      ...(ca.narrative_report
        ? [ca.narrative_report]
        : [
          `MARKET OVERVIEW`,
          ...
          `Market Quality:    ${ca.quality_label || ""}`,
          `Market Leader:     ${ca.market_leader} (★${ca.market_leader_rating})`,
          `Most Reviewed:     ${ca.most_reviewed}`,
          ``,
          `MARKET INSIGHTS`,
          `${"-".repeat(40)}`,
          ...(ca.market_insights || []).map((i, n) => `  ${n + 1}. ${i}`),
          ``,
          `STRATEGIC RECOMMENDATIONS`,
          `${"-".repeat(40)}`,
          ...(ca.recommendations || []).map((r, n) => `  ${n + 1}. ${r}`),
          ``,
            ...
            <div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(160px, 1fr))", gap: 12, marginBottom: 20 }}>
                {[
                  ...
                  { label: "Market Quality",    value: competitiveAnalysis.quality_label,         color: "#a78bfa", small: true },
                  ...
                  { label: "Most Reviewed",     value: competitiveAnalysis.most_reviewed,         color: "#3b82f6", small: true },
                  { label: "Avg Reviews",       value: (competitiveAnalysis.average_reviews || 0).toLocaleString(), color: "#6b7280" },
              ...
              {competitiveAnalysis.recommendations?.length > 0 && (
                <div style={{
                  background: "rgba(147,51,234,0.06)", borderRadius: 12, padding: 18,
                  border: "1px solid rgba(147,51,234,0.2)", marginBottom: 20, marginTop: 20,
                }}>
                  <h4 style={{ margin: "0 0 14px", color: "#a78bfa", fontSize: "0.8rem", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                    Strategic Recommendations
                  </h4>
                  <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                    {competitiveAnalysis.recommendations.map((rec, i) => (
                      <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                        ...
                        <p style={{ margin: 0, fontSize: "0.85rem", color: "#d1d5db", lineHeight: 1.6 }}>{rec}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {competitiveAnalysis.narrative_report && (
                <div style={{
                  background: "#111827", borderRadius: 12, padding: 20,
                  border: "1px solid #374151", marginBottom: 20, marginTop: 20,
                }}>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 14 }}>
                    <h4 style={{ margin: 0, color: "#a78bfa", fontSize: "0.8rem", textTransform: "uppercase", letterSpacing: "0.06em" }}>
                      Full Narrative Report
                    </h4>
                    <button
                      onClick={() => {
                        const blob = new Blob([competitiveAnalysis.narrative_report], { type: "text/plain" });
                        const a = document.createElement("a");
                        a.href = URL.createObjectURL(blob);
                        a.download = `competitive-report-${Date.now()}.txt`;
                        a.click();
                      }}
                      style={{
                  ...
                  <pre style={{
                    ...
                    fontSize: "0.75rem", color: "#9ca3af", lineHeight: 1.7,
