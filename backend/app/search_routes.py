...
search_routes.py
----------------
Unified search pipeline supporting two report types:

SWOT Analysis (id: "swot")
  1. Receive keyword, report_type, city, country, radius_km, place_name.
  2. If place_name provided: inject it into each quadrant prompt so Gemini
     includes a comparison section for that specific establishment.
  3. Resolve city coordinates, fetch inscriber tiles, build absolute coords.
  4. Split coordinates into 4 quadrants (N/E/S/W) using dominant-axis method.
  5. One Gemini call per quadrant + one dedicated cafe SWOT call if place_name set.

Competitive SWOT Analysis (id: "competitive_swot")
  1. Receive keyword, report_type, city, country, radius_km, place_name.
  2. place_name is required — user types it directly, no URL extraction.
  3. Skip inscriber entirely — Gemini reasons about the full radius area.
  4. Single Gemini call: analyse the user's cafe against ~100 competitors.
...
# key  : str  task_id (uuid4)
# value: dict with running, progress, results, error, status_message, …
# ---------------------------------------------------------------------------
search_tasks: dict = {}
...
# ---------------------------------------------------------------------------
# Shared Gemini key + model rotator
# ---------------------------------------------------------------------------
try:
...
# ---------------------------------------------------------------------------
# Keyword list — add more entries here as needed
# These are shown as dropdown options in the frontend.
# ---------------------------------------------------------------------------
KEYWORDS: List[str] = [
    "Cafes",
    "Restaurants",
...
# ---------------------------------------------------------------------------
# Helper: extract business name from a Google Maps URL
# ---------------------------------------------------------------------------
...
def _extract_place_name_from_url(url: str) -> Optional[str]:
    ...
    if not any(domain in url for domain in ["google.com/maps", "maps.app.goo.gl", "goo.gl/maps"]):
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.warning(
                f"_extract_place_name: non-200 response {resp.status_code}"
            )
            return None

        html = resp.text
        logger.info(
            f"_extract_place_name: HTML snippet (first 500): "
            f"'{html[:500]}'"
        )
        ...
        if title_match:
            ...
            logger.info(
                f"_extract_place_name: after cleaning = '{cleaned}'"
            )

            if cleaned and len(cleaned) > 2:
                logger.info(
                    f"_extract_place_name: SUCCESS via title = '{cleaned}'"
                )
                return cleaned
        else:
        ...
        if place_match:
            ...
            logger.info(
                f"_extract_place_name: SUCCESS via URL path = '{name}'"
            )
        ...
        logger.warning(
            "_extract_place_name: both strategies failed — returning None"
        )
        return None
...
def _get_city_coordinates(country: str, city: str) -> Optional[tuple]:
    ...
    Returns None if the country file is not found or the city is not listed.
    """
    try:
        country_files = {
        ...
        for entry in data:
            if entry.get("ASCII Name", "").lower() == city.lower():
                ...
                logger.info(f"Resolved coordinates for {city}, {country}: ({lat}, {lon})")
                return (lat, lon)
...
def _fetch_tiles(bounds: tuple) -> List[tuple]:
    ...
    try:
        ...
        if isinstance(data, dict) and "raw_coordinates" in data:
            ...
            logger.info(f"Inscriber returned {len(flat)} tiles (raw_coordinates format)")
            return flat
...
def _build_target_coords(center: tuple, tiles: List[tuple]) -> List[tuple]:
    ...
    logger.info(f"Built {len(targets)} absolute coordinates from center + {len(tiles)} tiles")
    return targets
...
def _call_gemini_and_parse(prompt_text: str, batch_number: int) -> dict:
    ...
    if not GEMINI_AVAILABLE:
        ...
        result["items"]  = [{"name": f"Mock {batch_number}-{i}", "info": "No key"}
                            for i in range(2)]
        result["status"] = "mock"
        return result
...
def _run_search_task(
    ...
    try:
        ...
        place_name_log = f'"{place_name}"' if place_name else "None (skipping cafe SWOT)"
        logger.info(
            f"[SEARCH {task_id}] Step 6 check: place_name={place_name_log}"
        )
        if place_name:
            logger.info(
        ...
        logger.error(
            f"[SEARCH {task_id}] Unexpected error: {traceback.format_exc()}"
        )
...
async def start_search(request: SearchRequest, background_tasks: BackgroundTasks):
    ...
    if not request.city.strip() or not request.country.strip():
        return JSONResponse(
            status_code=400,
            content={"error": "city and country are required"}
        )
    ...
    if request.report_type == "competitive_swot" and not (request.place_name or "").strip():
        return JSONResponse(
            status_code=400,
            content={"error": "Establishment name is required for Competitive SWOT Analysis."}
        )
